"""
CLI entry-point for the MultiModal Flood Prediction pipeline.

Usage
-----
    # Train
    python -m flood_model --mode train --data data/flood_dataset.geojson

    # Predict
    python -m flood_model --mode predict --data data/new_sites.geojson \
...
logger = logging.getLogger("flood_model")
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MultiModal Late-Fusion Flood Predictor",
    )
    p.add_argument(
        "--mode",
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Folder with flood GeoJSON samples (default: 'flood datasets')",
    )
    p.add_argument(
        "--non-flood-dir",
        type=str,
        default=None,
        help="Folder with non-flood GeoJSON samples (default: 'non-flood datasets')",
    )
    p.add_argument(
        "--predict-dir",
        type=str,
...
def run_predict(cfg: Config, predict_dir: str | None = None) -> None:
    ...
    logger.info(
        "Summary  │  n=%d  flood=%d  no-flood=%d  "
